...
namespace Interview.DesignPatterns
{
    class SingletonDesignPattern
    {
        ...
        //As you can see, this is extremely simple - but why is it thread-safe and how lazy is it? Well, static constructors in C# are specified to execute only when an
        //instance of the class is created or a static member is referenced and to execute only once per AppDomain. Given that this check for the type being newly
        //constructed needs to be executed whatever else happens, it will be faster than adding extra checking as in the previous examples. There are couple of wrinkles,
        //however:
        ...
        //is fully lazy, but has all the performance benefits of the previous ones. Note that although nested classes have access to the enclosing class's private members,
        //the reverse is not true, hence the need for instance to be internal here. That doesn't raise any other problems, though, as the class itself is private.
        //The code is more complicated in order to make the instantiation lazy, however.
        ...
        //into a method fetching the singleton, etc. However, it does show an important point. On my laptop, the slowest solution (by a factor of about 5) is the locking
        //one (solution 2). Is that important? Probably not, when you bear in mind that it still managed to acquire the singleton a billion times in under 40 seconds.
        //(Note: this article was originally written quite a while ago now - I'd expect better performance now). That means that if you're "only" acquiring the singleton
        //four hundred thousand times per second, the cost of the acquisition is going to be 1% of the performance - so improving it isn't going to do a lot. Now,
        //if you are acquiring the singleton that often - isn't it likely you're using it within a loop? If you care that much about improving the performance a little
        //bit, why not declare a local variable outside the loop, acquire the singleton once and then loop. Bingo, event the slowest implementation becomes easily
        //adequate.
        ...
        //Different runtimes handle this case differently, but I don't know of any which do the desired thing (running the type initializer again), and even if one did,
        //your code would be broken on other runtimes. To avoid these problems, I'd suggest using the second pattern listed on the page - just use a simple lock, and
        //go through the check each time, building the instance in the method/property if it hasn't already been successfully built.

        //Conclusion
        //There are various different ways of implementing the singleton pattern in C#. A reader has written to me detailing a way he has encapsulated the synchronization
        //aspect, which while I acknowledge may be useful in a few particular situations (specifically where you want very high performance and the ability to determine
        //whether or not the singleton has been created and full laziness regardless of other static members being called). I don't personally see that situation coming
        //up often enough to merit going further with on this page.
